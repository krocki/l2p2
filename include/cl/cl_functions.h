...
std::string cl_reduce (cl_array<float>& y, cl_array<float>& x, std::string reduce_op) {
	...
	cl_config_string += " n=" + std::to_string(n) + " w=" + std::to_string(workgroups) + " g=" + std::to_string(global_work_size) + " l=" + std::to_string(local_work_size) + " i=" + std::to_string(internal_iterations);
